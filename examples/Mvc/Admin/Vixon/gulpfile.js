...
gulp.task('clean:Vixon', function (callback) {
    del.sync(paths.src.libs.dir);
    callback();
});

gulp.task('build', gulp.series(gulp.parallel('clean:Vixon', 'copy:libs'), 'scss'));
gulp.task('default', gulp.series(gulp.parallel('clean:Vixon', 'copy:libs', 'scss'), gulp.parallel('watch')));
