...
import { useUpdateBuyerStage } from '@/hooks/useUpdateBuyerStage'
import { useDeleteBuyerStage } from '@/hooks/useDeleteBuyerStage'
import type { BuyerCategory } from '@/hooks/useBuyerCategories'

interface EditBuyerStageModalProps {
  show: boolean
  onHide: () => void
  categoryId: string
  currentStages: BuyerCategory['stages']
  stageId: string
  onStageUpdated?: (category: BuyerCategory) => void
  onStageDeleted?: () => void
}

const EditBuyerStageModal = ({ show, onHide, categoryId, currentStages, stageId, onStageUpdated, onStageDeleted }: EditBuyerStageModalProps) => {
  const [showDeleteConfirm, setShowDeleteConfirm] = useState(false)
  
  const { control, updateStage, loading, reset } = useUpdateBuyerStage(categoryId, currentStages, stageId, (category) => {
    onHide()
    if (onStageUpdated) {
  ...
  const { deleteStage, loading: deleteLoading } = useDeleteBuyerStage(() => {
    onHide()
    if (onStageDeleted) {
      onStageDeleted()
    }
        ...
        <ModalBody>
          ...
          {showDeleteConfirm && (
            <div className="alert alert-danger mt-3">
              ...
              <p className="small mb-2 text-muted">Покупатели, привязанные к этой стадии, останутся, но стадия будет скрыта.</p>
              <div>
                <Button 
                  variant="danger" 
                  size="sm" 
...
export default EditBuyerStageModal
