...
import { useAddDealStage } from '@/hooks/useAddDealStage'
...
interface AddDealStageModalProps {
  ...
  onStageAdded?: (category: DealCategory) => void
}

const AddDealStageModal = ({ show, onHide, categoryId, currentStages, onStageAdded }: AddDealStageModalProps) => {
  const { control, addStage, loading, reset, nextOrder } = useAddDealStage(categoryId, currentStages, (category) => {
    onHide()
    if (onStageAdded) {
