...
const AddDealModal = ({ show, onHide, categoryId, stageId, onDealCreated }: AddDealModalProps) => {
  const { user } = useAuthContext()
  const defaultResponsibleUserId = user?.id || ''
  const { control, createDeal, loading, reset } = useCreateDeal(categoryId, stageId, defaultResponsibleUserId, (deal) => {
    onHide()
    if (onDealCreated) {
      onDealCreated(deal)
    }
