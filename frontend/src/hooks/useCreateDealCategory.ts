...
interface CreateDealCategoryApiResponse {
  status: boolean
  data?: DealCategory
  message?: {
    text?: string
...
export const useCreateDealCategory = (onSuccess?: (category: DealCategory) => void) => {
  ...
  const createCategory = handleSubmit(async (values: CreateCategoryFormFields) => {
    setLoading(true)
    try {
      const response: AxiosResponse<CreateDealCategoryApiResponse> = await httpClient.post('/deals/categories', {
        name: values.name,
