...
export const useAddBuyerStage = (categoryId: string, currentStages: BuyerCategory['stages'], onSuccess?: (category: BuyerCategory) => void) => {
  ...
  const addStage = handleSubmit(async (values: AddStageFormFields) => {
    ...
    try {
      ...
      // ВАЖНО: для существующих стадий передаём их id, чтобы сохранить связь с покупателями
      const newStages = [
      ...
      const response: AxiosResponse<UpdateStagesApiResponse> = await httpClient.put(`/buyers/categories/${categoryId}/stages`, {
        stages: newStages,
      })
      ...
      console.error('Error adding buyer stage:', err)
