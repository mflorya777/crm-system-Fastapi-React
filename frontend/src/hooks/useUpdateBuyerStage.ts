...
export const useUpdateBuyerStage = (
  categoryId: string,
  currentStages: BuyerCategory['stages'],
  stageId: string,
  onSuccess?: (category: BuyerCategory) => void,
) => {
  ...
  const updateStage = handleSubmit(async (values: UpdateStageFormFields) => {
    ...
    try {
      ...
      // ВАЖНО: передаём id всех стадий, чтобы сохранить связь с покупателями
      ...
      console.error('Error updating buyer stage:', err)
